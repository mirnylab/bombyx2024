    ...
    if np.random.random() < (1/lifetime):
        ...
        while True:
            ...
            if (context[a:a+len(walker)] == 0).all():
                ...
                return 
def extrusion(d_AB, d_X, lambda_, chain_list, n_steps, mon_types):
...
lambda_ = 100
d_X = 50
dAB_list = [3600]#[0, 50, 100, 200, 500, 400, 1000, 5000]

# Defining params for LEF dynamics
n_steps = 500_000 # Duration of the simulation

extrusion_input_list = [(d_AB, d_X, lambda_) for d_AB in dAB_list]

with mp.Pool(len(dAB_list)) as p:
    f = partial(extrusion, 
